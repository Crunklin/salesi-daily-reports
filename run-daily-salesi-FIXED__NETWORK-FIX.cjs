...
async function clickApply(page) {
  ...
  for (const loc of candidates) {
    ...
    if (await loc.isVisible({ timeout: 3000 }).catch(() => false)) {
      // FIXED: Don't wait for networkidle - just click and wait for DOM to load
      await Promise.all([
        page.waitForLoadState('domcontentloaded', { timeout: 15000 }),
        loc.click({ timeout: 10000 })
      ]);
      
      // Give UI a moment to stabilize after the report loads
...
async function run() {
  ...
  try {
    ...
    await safe('Open COR', async () => {
      if (await openCORFromWelcome(page)) {
        await page.waitForLoadState('domcontentloaded');
        await page.waitForTimeout(2000);
        return true;
      }
      throw new Error('Could not open Call Outcome Report');
    });

    // Open filter panel once for ALL reps
    log('Opening Filter panel…');
    const filterOpen = await ensureFilterPanel(page);
    if (!filterOpen) {
      throw new Error('Could not open Filter panel');
    }
    
    // Take debug screenshot after opening filter panel
    const debugShotPath = path.join(OUT_DIR, `debug-filter-ready-${RUN_ID}.png`);
    ...
    for (let i = 0; i < REPS.length; i++) {
      ...
      await safe('Ensure filter panel open', async () => {
        const ctx = await resolveCRMFrame(page);
        const panelOpen = await ensureFilterPanel(page);
        if (!panelOpen) {
          throw new Error(`Filter panel not accessible for ${rep.name}`);
        }
      });

      // Set dates
      await safe('Set dates', async () => {
        await setDateInputs(page, dateStr);
      });

      // Set user - with improved verification
      await safe('Set user', async () => {
        const success = await setUser(page, rep);
        if (!success) {
          throw new Error(`Failed to set user ${rep.name}`);
        }
        
        // CRITICAL FIX: Wait for UI to stabilize after user selection
        log('  Waiting for UI to stabilize after user selection...');
        await page.waitForTimeout(1000);
      });

      // Click Apply - with improved wait logic
      await safe('Click Apply', async () => {
        const ctx = await resolveCRMFrame(page);
        const success = await clickApply(page);
        if (!success) {
          throw new Error('Failed to click Apply');
        }
      });

      // Wait for report to load (replace old networkidle wait)
      await page.waitForTimeout(4000);

      // Try to open detail view
      const filePath = path.join(OUT_DIR, `call-outcome_${rep.name.toLowerCase()}_${dateStr.replace(/\//g, '-')}.png`);
      
      let openedDetail = false;
      await safe('Open detail link', async () => {
        const ctx = await resolveCRMFrame(page);
        
        // Look for the "Click for detail" link
        const detailLink = ctx.locator('a').filter({ hasText: /Click for detail/i });
        ...
        if (await detailLink.isVisible({ timeout: 2000 }).catch(() => false)) {
          ...
          // Verify we're on the detailed view
          const onDetailPage = await page.locator('h1:has-text("CALLS MADE:")').isVisible({ timeout: 3000 }).catch(() => false);
        ...
        // If opening detail fails, log and continue
        log(`  No call details for ${rep.name} on ${dateStr} — skipping but will navigate back for next rep.`);
        
        // Send alert email for skipped rep
        sendAlert({
          subject: `No calls for ${rep.name} on ${dateStr}`,
          text: `The rep ${rep.name} had no calls on ${dateStr}, so no report was generated.\n\nError: ${err?.message || err}`
        }).catch(() => {});
