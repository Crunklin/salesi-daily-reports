...
async function clickFilterBar(page) {
  ...
  const candidates = [
    ctx.locator('#ctl00_divFilter1').getByText('Filter'),
    ctx.locator('.accordion__title').filter({ hasText: /Filter/i }),
    ctx.getByRole('button', { name: /^Filter$/i }),
    page.getByText(/^Filter$/i),
    ctx.locator('button:has-text("Filter")'),
    ctx.locator('a:has-text("Filter")'),
    ctx.locator('div.crm-filter, .crm-filter').filter({ hasText: /Filter/i }),
    ctx.locator('[aria-label*="Filter" i]'),
    ctx.locator('[title*="Filter" i]')
  ];
...
async function isFilterPanelOpen(page) {
  ...
  const checks = await Promise.all([
    ctx.locator('#start-date').isVisible({ timeout: 1000 }).catch(() => false),
    ctx.locator('#end-date').isVisible({ timeout: 1000 }).catch(() => false),
    ctx.locator('#ddUser').isVisible({ timeout: 1000 }).catch(() => false),
  ]);
...
async function ensureFilterPanel(page) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    log(`  Attempt ${attempt} to open filter panel...`);
    
    if (await clickFilterBar(page)) {
  ...
  log('  ✗ Failed to open filter panel after 3 attempts');
  return false;
}

async function setDates(page, dateStr) {
  const ctx = await resolveCRMFrame(page);
  ...
  await safe('Set start date', async () => {
    ...
    try {
      const startEl = ctx.locator('#start-date');
    ...
    if (!success) {
      try {
        const startEl = ctx.locator('input[name="start-date"]');
        if (await startEl.isVisible({ timeout: 2000 })) {
          await startEl.clear();
          await startEl.fill(dateStr);
    ...
    if (!success) {
      try {
        const startEl = ctx.getByLabel('Start Date:', { exact: false });
        if (await startEl.isVisible({ timeout: 2000 })) {
          await startEl.clear();
          await startEl.fill(dateStr);
  ...
  await safe('Set end date', async () => {
    ...
    // Method 1: Direct ID selector
    try {
      const endEl = ctx.locator('#end-date');
    ...
    if (!success) {
      try {
        const endEl = ctx.locator('input[name="end-date"]');
        if (await endEl.isVisible({ timeout: 2000 })) {
          await endEl.clear();
          await endEl.fill(dateStr);
    ...
    if (!success) {
      try {
        const endEl = ctx.getByLabel('End Date:', { exact: false });
        if (await endEl.isVisible({ timeout: 2000 })) {
          await endEl.clear();
          await endEl.fill(dateStr);
  ...
  await safe('Dismiss calendar', async () => {
    ...
    const pageTitle = ctx.locator('h1.page-title');
...
async function setUser(page, rep) {
  ...
  try {
    const userSelect = ctx.locator('#ddUser');
    if (await userSelect.isVisible({ timeout: 2000 })) {
      await userSelect.selectOption(rep.id);
      
      // CRITICAL FIX: Wait for UI to update after selection
      await page.waitForTimeout(1000);
      
      // CRITICAL FIX: Verify the selection actually worked
      const selectedValue = await userSelect.inputValue();
      if (selectedValue === rep.id) {
        log(`  ✓ User set via main dropdown (verified)`);
        return true;
      } else {
        log(`  ✗ Main dropdown selection failed verification (got: ${selectedValue}, expected: ${rep.id})`);
      }
    }
  } catch (e) {
  ...
  try {
    const found = await ctx.evaluate(({ id, name }) => {
    ...
    if (found) {
      // CRITICAL FIX: Wait for UI to update after JavaScript selection
      await page.waitForTimeout(1000);
      
      // CRITICAL FIX: Try to verify the selection
      const userSelect = ctx.locator('#ddUser');
      if (await userSelect.isVisible({ timeout: 1000 }).catch(() => false)) {
        const selectedValue = await userSelect.inputValue();
        if (selectedValue === rep.id) {
          log(`  ✓ User set via JavaScript (verified)`);
          return true;
        } else {
          log(`  ✗ JavaScript selection failed verification (got: ${selectedValue}, expected: ${rep.id})`);
          // Don't return true - fall through to fail
        }
      } else {
        // Can't verify, but JavaScript said it worked
        log(`  ✓ User set via JavaScript (unable to verify)`);
        return true;
      }
    }
  } catch (e) {
...
async function clickApply(page) {
  ...
  // CRITICAL FIX: Wait a bit before trying to click Apply
  await page.waitForTimeout(500);
  
  const candidates = [
    ctx.locator('#ctl00_btnApply'),
    ctx.getByRole('button', { name: /^Apply Filters$/i }),
    ctx.locator('input[value="Apply Filters"]'),
    ctx.locator('button.apply'),
    ctx.locator('.apply')
  ];
  
  for (const loc of candidates) {
    // CRITICAL FIX: Increased timeout to wait for button to become visible
    if (await loc.isVisible({ timeout: 3000 }).catch(() => false)) {
      // MAJOR FIX: Don't wait for networkidle - the page has background activity
      // that prevents it from ever reaching networkidle state, causing 30s timeouts
      await loc.click();
      
      // Wait for the results to load - look for either:
      // 1. The summary table (Sales Rep Name column) on landing page
      // 2. Any indication the filter has applied
      await page.waitForTimeout(3000); // Give the page time to process the filter
      
      // Verify the filter actually applied by checking for content
      const hasResults = await Promise.race([
        ctx.locator('th:has-text("Sales Rep Name")').isVisible().catch(() => false),
        ctx.locator('table').isVisible().catch(() => false),
        ctx.locator('.grid-data').isVisible().catch(() => false),
      ]).catch(() => false);
      
      if (!hasResults) {
        log('  ⚠ Apply clicked but no results visible, waiting longer...');
...
async function run() {
  ...
  try {
    ...
    const opened = await safe('Open COR from welcome', () => openCORFromWelcome(page));
    if (!opened) throw new Error('Could not find/click "Call Outcome Report"');
    await page.waitForLoadState('domcontentloaded').catch(() => {});

    // Open filter panel
    log('Opening Filter panel…');
    const panelOpen = await ensureFilterPanel(page);
    if (!panelOpen) {
      await page.screenshot({ 
        path: path.join(OUT_DIR, 'DEBUG_filter_panel_not_opening.png'),
        fullPage: true 
      });
    ...
    await page.screenshot({ path: path.join(OUT_DIR, 'DEBUG_filter_panel_open.png'), fullPage: true });
    log('Debug screenshot taken');
    ...
    for (let i = 0; i < REPS.length; i++) {
      ...
      // Check filter panel state before each rep
      const panelNow = await isFilterPanelOpen(page);
      if (!panelNow) {
        log('  Filter panel closed, attempting to reopen...');
        const reopened = await ensureFilterPanel(page);
        if (!reopened) throw new Error('Failed to reopen filter panel before processing rep');
      }

      await setDates(page, dateStr);
      
      const userOk = await safe('Set user', () => setUser(page, rep));
      if (!userOk) {
        await page.screenshot({ path: path.join(OUT_DIR, `DEBUG_${rep.name}_user_not_set.png`), fullPage: true });
        throw new Error(`Could not set user for ${rep.name}`);
      }
      
      // CRITICAL FIX: Additional wait after user selection to ensure UI is stable
      log('  Waiting for UI to stabilize after user selection...');
      await page.waitForTimeout(1000);

      const applyOk = await safe('Click Apply', () => clickApply(page));
      if (!applyOk) {
        await page.screenshot({ path: path.join(OUT_DIR, `DEBUG_${rep.name}_apply_failed.png`), fullPage: true });
        throw new Error(`Could not click Apply for ${rep.name}`);
      }

      // Wait for results + see if there's a detail link
      await page.waitForTimeout(4000);

      const filePath = path.join(OUT_DIR, `call-outcome_${rep.name.toLowerCase().replace(/\s+/g, '-')}_${dateStr.replace(/\//g, '-')}.png`);

      // Try clicking detail link
      const openedDetail = await safe('Open detail link', async () => {
        const detailLink = page.locator('a').filter({ hasText: /Click for detail/i });
        if (await detailLink.isVisible({ timeout: 3000 })) {
          ...
          const onDetailPage = await page.locator('h1').filter({ hasText: /CALLS MADE:/i }).isVisible({ timeout: 5000 }).catch(() => false);
          if (!onDetailPage) {
        ...
        log(`  No call details for ${rep.name} on ${dateStr} — skipping but will navigate back for next rep.`);
        
        // Take debug screenshot
        page.screenshot({ 
          path: path.join(OUT_DIR, `debug-no-details-${rep.name}.png`),
          fullPage: true 
        }).catch(() => {});
        
        // Send alert email
        sendAlert({
          subject: `ALERT: No call details for ${rep.name} on ${dateStr}`,
          text: `No call detail link found for ${rep.name} on ${dateStr}.\n\nError: ${err?.message || err}`
        }).catch(() => {});
        
        return false;
      });
      
      // If we couldn't open detail view, navigate back and continue to next rep
      if (!openedDetail) {
        // CRITICAL: Navigate back to landing page before skipping to next rep
        if (i < REPS.length - 1) {
          log('  Skipped rep - navigating back before next rep...');
          await safe('Return to COR landing after skip', async () => {
        ...
        continue; // NOW we can skip to next rep
      }
      ...
      if (i < REPS.length - 1) {
        ...
        const ctx = await resolveCRMFrame(page);
        const userDropdownReady = await ctx.locator('#ddUser').isVisible({ timeout: 3000 }).catch(() => false);
