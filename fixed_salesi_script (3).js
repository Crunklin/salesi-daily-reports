...
async function run() {
  ...
  try {
    ...
    for (let i = 0; i < REPS.length; i++) {
      ...
      // Check if there are any calls for this rep
      const hasCallsToReport = await safe('Check for calls and open detail', async () => {
        // First check if the "Click for detail" link exists (meaning there are calls)
        ...
        if (!clicked) {
          // Check if the count shows 0 calls
          const zeroCallsText = await page.locator('#CreatedCount').textContent().catch(() => '');
          if (zeroCallsText.includes('0 ') || zeroCallsText.trim() === '0') {
            log(`  No calls found for ${rep.name} - skipping`);
            return false; // Return false to indicate skip
          }
          
          // If not zero but still no detail link, something is wrong
          await page.screenshot({ path: path.join(OUT_DIR, `debug-no-detail-link-${rep.name}.png`) });
          throw new Error('Detail link not found and call count is not zero - check debug screenshot');
        }
        
        // Wait for the detailed page to load
        await page.waitForLoadState('networkidle', { timeout: 10000 });
        
        // Verify we're on the detailed page
        ...
        return true; // Return true to indicate we should take screenshot
      });
      
      // Skip screenshot and email if no calls were found
      if (!hasCallsToReport) {
        log(`Skipped ${rep.name} - no calls to report`);
        // Continue to next rep without taking screenshot or sending email
        if (i < REPS.length - 1) {
          // Still need to navigate back for next rep
          log('  Navigating back for next rep...');
    ...
    try {
      const fatalHtml = path.join(OUT_DIR, `FATAL_${RUN_ID}.html`);
