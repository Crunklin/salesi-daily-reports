...
async function setUser(page, rep) {
  ...
  try {
    ...
    if (found) {
      ...
      // CRITICAL FIX: Verify the selection worked
      const userSelect = ctx.locator('#ddUser');
...
async function run() {
  ...
  try {
    ...
    for (let i = 0; i < REPS.length; i++) {
      ...
      // Make sure Filter panel is open for each rep before setting dates
      const panelOpen = await ensureFilterPanel(page);
      if (!panelOpen) throw new Error('Could not open Filter panel at start of rep');

      // Dates (with one-time recovery retry if calendar/panel got stuck)
      try {
        await setDateInputs(page, dateStr);
      } catch (err) {
        log(`  setDateInputs failed once: ${err?.message || err} – reopening panel and retrying...`);
        await ensureFilterPanel(page);
        await page.waitForTimeout(500).catch(() => {});
        await setDateInputs(page, dateStr); // let this throw if it still fails
      }
      ...
      // CRITICAL FIX: Additional wait after setting user before clicking Apply
      ...
      await safe('Early zero-calls check', async () => {
        ...
        if (createdCount === 0) {
          _earlySkipZeroCalls = true;
        }
      });

      if (_earlySkipZeroCalls) {
        const skipShot = path.join(OUT_DIR, `SKIPPED_zero-calls_${rep.name}_${dateStr.replace(/\//g, '-')}.png`);
        try { await page.screenshot({ path: skipShot, fullPage: true }); } catch {}
        log(`  Zero calls for ${rep.name} on ${dateStr} – skipping to next rep.`);
        await sendAlert({
          subject: `Skipped ${rep.name} – zero calls on ${dateStr} (RunID ${RUN_ID})`,
          text: `Sales-i automation skipped ${rep.name} early because CreatedCount (or equivalent) evaluated to 0.`,
          attachments: [{ filename: path.basename(skipShot), path: skipShot }]
        });
        continue;
      }
      ...
      try {
        await safe('Open detail link', async () => {
          ...
          await ctx.waitForLoadState('networkidle', { timeout: 10000 });
          
          // Verify we're on the detailed page by checking for customer data
          const hasDetailData = await ctx.waitForSelector('td:has-text("Customer")', { timeout: 5000 }).catch(() => false);
          if (!hasDetailData) {
            await ctx.screenshot({ path: path.join(OUT_DIR, `debug-wrong-page-${rep.name}.png`) });
        ...
        // Only skip on our explicit "no detail link" or "not on detailed page" errors
        if (/Detail link not found/i.test(msg) || /Not on detailed call notes page/i.test(msg)) {
          const skipShot = path.join(OUT_DIR, `SKIPPED_no-calls_${rep.name}_${dateStr.replace(/\//g, '-')}.png`);
          try { await page.screenshot({ path: skipShot, fullPage: true }); } catch {}
          log(`  No call details for ${rep.name} on ${dateStr} – skipping but will navigate back for next rep.`);
          await sendAlert({
            subject: `Skipped ${rep.name} – no calls on ${dateStr} (RunID ${RUN_ID})`,
            text: `Sales-i automation skipped ${rep.name} because the "Click for detail" link wasn't present (likely no calls yesterday).`,
            attachments: [{ filename: path.basename(skipShot), path: skipShot }]
          });
          openedDetail = false; // Mark as not opened, but don't continue yet
        } else {
          // rethrow for unexpected errors so the top-level handler can alert
          throw err;
        }
      }
      
      // If we didn't open detail successfully, we need to navigate back before continuing
      if (!openedDetail) {
