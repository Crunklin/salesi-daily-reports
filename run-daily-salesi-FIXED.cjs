...
; // delimiter to protect against ASI
...
async function resolveCRMFrame(page) {
  try {
    ...
    const hasControlsOnPage = await Promise.race([
      ...
      ctx.getByRole('button', { name: /Apply/i }).isVisible().catch(() => false),
    ]).catch(() => false);
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      try {
        await ctx.evaluate(u => { window.location.assign(u); }, url);
...
async function openCORFromWelcome(page) {
  ...
  const card = ctx.locator('section,div,li,article').filter({ hasText: /Call Outcome Report/i }).first();
  ...
  const nth = ctx.getByRole('link', { name: /^VIEW REPORT$/i }).nth(3);
  if (await nth.isVisible().catch(() => false)) { await nth.click(); return true; }
  // Last resort: any direct link
  const direct = ctx.getByRole('link', { name: /Call Outcome Report/i });
...
async function run() {
  ...
  try {
    ...
    await ctx.getByRole('textbox', { name: 'UserName' }).fill(SI_USERNAME);
    await ctx.getByRole('button', { name: 'Next' }).click();
    await ctx.getByRole('textbox', { name: 'Password' }).fill(SI_PASSWORD);
...
try {
  ...
  if (/Detail link not found/i.test(msg) || /Not on detailed call notes page/i.test(msg)) {
    ...
    try { await page.screenshot({ path: skipShot, fullPage: true }); } catch {}
    log(`  No call details for ${rep.name} on ${dateStr} — skipping to next rep.`);
    await sendAlert({
      ...
      attachments: [{ filename: require('path').basename(skipShot), path: skipShot }]
    });
    continue; // move on to next rep
  } else {
    // rethrow for unexpected errors so the top-level handler can alert
    throw err;
  }
}
if (!openedDetail) continue;
;

      // Columns only for first rep
      if (i === 0) {
        await safe('Adjust columns (first rep only)', async () => {
          const columnsBtn = ctx.getByRole('button', { name: /^Columns$/i });
          if (await columnsBtn.isVisible({ timeout: 5000 })) {
            await columnsBtn.click();
            await ctx.getByRole('checkbox', { name: /^Contact$/i }).uncheck({ force: true }).catch(() => {});
            await ctx.getByRole('checkbox', { name: /^Call Outcome$/i }).uncheck({ force: true }).catch(() => {});
            await ctx.getByRole('checkbox', { name: /^Next Action$/i }).uncheck({ force: true }).catch(() => {});
            await ctx.getByRole('button', { name: /^OK$/i }).click().catch(() => {});
      ...
      if (i < REPS.length - 1) {
        await safe('Return to COR landing', async () => {
          const link = ctx.getByRole('link', { name: /^Call Outcome Report$/i });
          if (await link.isVisible().catch(() => false)) { 
            await link.click(); 
            await page.waitForLoadState('domcontentloaded');
            return; 
          }
          const li = ctx.getByRole('listitem', { name: /Call Outcome Report/i });
        ...
        if (!panelAgain) throw new Error('Could not re-open Filter panel for next rep');
      }
    }
